// 말판 css
$("td span").parent().css("border", "2px solid black");
$("#pos1").parent().css("background-color", "blue");

// 위치 무작위 전역 변수
var squareNum = $("td span").parent().length;
...
for (var i = 0; i < resetArray.length; i++) {
  $("td span").parent().eq(resetArray[i]).addClass("reset");
...
for (var i = 0; i < bonusArray.length; i++) {
  $("td span").parent().eq(bonusArray[i]).addClass("bonus");
...
$(".shuffle").on("click", function () {
  ...
  setTimeout(function () {
    ...
    $("#announce").html(id + "님의 말을 주사위" + res + "칸 만큼 이동합니다.");
    $("#alert").html("");
    for (var i = 0; i < 25; i++) {
      $("#pos" + i).html("");
    }
